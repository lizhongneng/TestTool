...
/**
 * This code is from
 * "packages/providers/ContactsProvider/src/com/android/providers/contacts/HanziToPinyin.java" in
 * android 4.2.2_r1
 */
...
public class HanziToPinyin {
    ...
    public static HanziToPinyin getInstance() {
        synchronized (HanziToPinyin.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                //FLYME:yangyatao@Feature modify for get Pinyin by Hanzi{@
    ...
    // FLYME:yangyatao@Feature add for get Pinyin by Hanzi{@
    public String transliterate(final String input) {
        ...
        for (Token token : tokenList) {
            ...
            if (DEBUG) {
                Log.d(TAG, "transliterate token.type = " + token.type + " token.source = "
                        + token.source + " token.target = " + token.target);
            }
        }
        String output = sb.toString();
        if (DEBUG) {
            Log.d(TAG, "transliterate input = " + input + " output = " + output);
        }
        return output;
    }
    // @}
